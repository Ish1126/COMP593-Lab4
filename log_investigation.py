...
def main():
    ...
    # Generate reports for ports that have 100 or more entries.
...
def tally_port_traffic():
    """Generates a dictionary where destination port numbers are keys and the values represent the frequency of occurrence.
...
def generate_port_traffic_report(port_number):
    """Creates a CSV report containing all network traffic for a specified destination port number.
...
def generate_invalid_user_report():
    """Generates a CSV report of all network activities indicating an attempt to log in as an invalid user."""
    capture_data = []
...
def generate_source_ip_log(ip_address):
    """Generates a plain text .log file containing all records that match a specified source IP address.
